...
func runCmdWithPipe() {
	fmt.Println("Run command `ps aux | grep php`: ")
	...
	if err := cmd1.Wait(); err != nil {
		fmt.Printf("Error: Couldn't waitG for the first command: %s\n", err)
		return
	}
	...
	if err := cmd2.Wait(); err != nil {
		fmt.Printf("Error: Couldn't waitG for the second command: %s\n", err)
		return
	}
	fmt.Printf("%s\n", outputBuf2.Bytes())
}
